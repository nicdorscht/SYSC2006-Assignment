...
_Bool dupe_id(unsigned short int *milestone_ids, unsigned short int *activity_ids, milestone_t *milestone_ptr)
{
	...
	{ // Loop over existing ids to check for duplicates
		if (milestone_ids[i] == 0)
		{ // Check if at end of existing ids, if so add id and break the loop
			milestone_ids[i] = milestone_ptr->id;
			break;
		}
		while (milestone_ptr->id == milestone_ids[i])
		{ // Id id is the same loop until user enters a unique one
	...
	{ // Loop over all activities in the milestone
		for (int j = 0; j < MAX_ACTIVITIES * MAX_MILESTONES; j++)
		{ // Loop over all existing activity ids
			if (activity_ids[j] == 0)
			{
			...
			while (milestone_ptr->activity_list[i].id == activity_ids[j])
